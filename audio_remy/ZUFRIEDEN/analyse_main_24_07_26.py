...
class Config:
    AUDIO_FILE = "korpus/peppa_long_tacos.mp3"
    MAX_DURATION = 1500
    HOP_LENGTH = 512
    FMIN = 80
    FMAX = 900
...
def compute_onset_results(t_onsets, rms_at_onsets, intensities):
    ...
    intensity_mean) - direkt aus dem RAM, ohne JSON-Umweg.
    """
    count = len(t_onsets)
...
def compute_voice_change_results(f0, t_pitch, rms_db, threshold_semitones=Auswertung.HALBTON_SCHWELLE):
    """
    Liefert die Kennzahlen, die calc_vcs() aus Auswertung braucht
    (voice_changes_per_minute, rms_mean_at_changes) - direkt aus dem RAM,
    ohne JSON-Umweg. Analog zu compute_onset_results() für den ACS.

    f0, t_pitch und rms_db sind hier framealigned (gleicher hop_length),
    genau wie beim Export in pitch.json - daher kann rms_db[i] direkt als
    Lautstärke am Stimmwechsel-Index i verwendet werden.
    """
    pitch_hz_list = [float(h) if not np.isnan(h) else None for h in f0]
    change_indices = Auswertung.detect_voice_changes(pitch_hz_list, threshold_semitones)
    ...
    if count > 0:
        rms_at_changes = [rms_db[i] for i in change_indices]
...
def analyze_file(audio_file, config=None):
    ...
    # lautstaerke_mittelwert = Auswertung.calc_lautstaerke_mittelwert(rms_db)  # nicht mehr Teil des Scores

    stimme_valenz = Auswertung.calc_stimme_score(pitch_results)
    # stimme_intensitaet = Auswertung.calc_stimme_intensitaet(pitch_results)  # nicht mehr Teil des Scores
    acs = Auswertung.calc_activity_score_normiert(onset_results)
    vcs = Auswertung.calc_vcs(voice_change_results)
    aufmerksamkeits_score = Auswertung.calc_aufmerksamkeits_score(
    ...
    return {
        ...
        # -------- Rohe Einzelwerte (für CSV/Korpus-Auswertung) --------
