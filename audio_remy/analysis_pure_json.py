...
def compute_onsets(y, sr, config, t_rms, rms_db):
    ...
    # RMS zum jeweiligen Onset-Zeitpunkt interpolieren
...
def export_analysis_json(t_onsets, intensities, rms_at_onsets, t_pitch, pitch_n, f0, output_dir="."):
    ...
    onset_data = {
        ...
        "summary": {
            ...
            "weighted_density":   round(float(count * np.mean(intensities)), 4)
        },
        "onsets": [
            {
                "index": i,
        ...
        "pitch": [
            {
                "index": i,
                "time_seconds": round(float(t), 4),
                "pitch_norm": round(float(p), 6),
                "pitch_hz": round(float(h), 4) if not np.isnan(h) else None
            }
            for i, (t, p, h) in enumerate(zip(t_pitch, pitch_n, f0))
        ]
    }
